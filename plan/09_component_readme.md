# 09 — Component README (draft)

**This file is a draft of `src/components/DataGrid/README.md`.** When we scaffold the component, copy the content below (everything under the `---` line) verbatim to that location, then iterate it there. Until the component exists, this file is the source of truth for integration docs.
...
Owns transition rules (sort/filter → reset page + clear row selection; all view changes → clear cell range), transient state (row/range selection, active editor), and assembles `gridProps` for you. You provide `view` + `columnConfig` as controlled state; the hook calls your change handlers.
...
- `DataGridColumnDef<TRow, TValue>` — column definition (`id`, `header`, `accessor`, `cell`, `align`, `editable`, `width`, pinning/visibility flags)
- `CellRenderer<TRow, TValue>` — `React.ComponentType<DataGridCellProps<TRow, TValue>>`
- `DataGridCellProps<TRow, TValue>` — everything a cell receives: display props + edit props (phase 2) + selection props + `extras`
...
- **Cell range selection** re-renders affected cells on every mouse-move during a drag. Fine at 4000 cells (100 × 40). Optimize with a store-based subscription if you push past that.
...
- Multi-column sort
- RTF inline editing (phase 1 renders a placeholder string)
...
A: Your fetch layer isn't keeping previous data. TanStack Query: set `keepPreviousData: true`. Otherwise: manually preserve `data` until the next result arrives.
