...
8. **Cell rendering is fully decoupled.** No type enum, no registry, no lookup. Each column declares its `cell` component inline on the column def (`column.cell?: CellRenderer<TRow, TValue>`). Falls back to `TextCell` which renders `String(value ?? '')` — ugly for non-strings, intentionally so. Phase 1 ships exactly one built-in cell: `TextCell`, which doubles as the fallback (there is no separate `DefaultCell`). Richer cells (`NumberCell`, `DateCell`, etc.) are deferred future work — consumers needing them write their own inline `cell` on the column def in the meantime. Cells receive a stable `DataGridCellProps` contract including the phase-2 editing seam (`isEditing`, `draftValue`, `setDraftValue`, `commitEdit`, `cancelEdit`).
